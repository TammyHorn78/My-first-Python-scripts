...
class ATM(EasyFrame):
    ...
    """def __init(self):
        EasyFrame.__init__(self, title = "ATM Login", width = 300, heeight = 200)
        self.failed_attemps = 0
        self.addLabel(text="Enter PIN:", row=0, column=0)
        self.pinField = self.addTextField(text="", row=0, column=1, password=True)
        self.loginButton = self.addButton(text="Login", row=1, column=0, columnspan=2, command=self.checkLogin)""" 
    ...
    def checklogin(self):
        ...
        if self.failed_attempts >= 3:
            ...
            messagebox.showwarning("Warning", "Police will be called due to multiple failed attempts!")
        else:
            messagebox.showerror("Error", f"Login failed! Attempt {self.failed_attempts} of 3.")
